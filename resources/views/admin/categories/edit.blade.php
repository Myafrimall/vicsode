...
<div class="mb-8">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mt-2">Edit Category: {{ $category->name }}</h1>
</div>

<form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data" class="max-w-2xl">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
        ...
        <div>
            ...
            @if($category->image)
                <div class="mb-3">
                    <img src="{{ asset($category->image) }}" alt="{{ $category->name }}" class="w-32 h-32 object-cover rounded-xl">
